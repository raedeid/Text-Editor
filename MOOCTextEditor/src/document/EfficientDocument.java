...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		Pattern tokSplitterWord = Pattern.compile("[a-zA-Z]+");
		Pattern tokSplitterSent = Pattern.compile("[!?.]+");
//		Pattern tokSplitterSyall = Pattern.compile("");
		...
		for (int i = 0; i < tokens.size(); i++) {
			...
			if (isWord(value)) {
				...
				if (tokSplitterWord.matcher(value).find()) {
					numWords++;
					numSyllables = numSyllables + countSyllables(value);
				}

			}else if (tokSplitterSent.matcher(value).find() ) {
					numSentences++;


			}
		}
//		Pattern tokSpillter  = Pattern.compile("");
